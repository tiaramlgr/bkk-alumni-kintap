...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        <div class="p-8 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div>
                <a href="{{ route('admin.dokumen.edit', $dokumen->id) }}" class="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-xl text-sm font-bold transition shadow-sm flex items-center gap-2">
                    <i class="fas fa-edit text-indigo-500"></i> Edit Data
                </a>
            </div>
        </div>
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
            <div class="space-y-5">
                ...
                <div>
                    ...
                    @else
                        <span class="text-rose-600 font-bold"><i class="fas fa-times-circle mr-1"></i> Tersembunyi (Disembunyikan dari Alumni)</span>
                    @endif
                </div>
            </div>
        ...
        <div class="bg-slate-50 p-6 border-t border-slate-100 flex justify-center">
            <a href="{{ asset('storage/' . $dokumen->path_file) }}" download="{{ $dokumen->nama_file }}" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-2xl font-bold transition shadow-lg shadow-blue-600/20 flex items-center gap-2">
                <i class="fas fa-download"></i> Unduh Dokumen Ke Perangkat
            </a>
        </div>
    </div>
</div>
